...
class FileChooser(gtk.FileChooserDialog):
	...
	def __init__(self, parent, func, default_path=None, files=False, save=False):
		""""""
		gtk.FileChooserDialog.__init__(self, "%s - %s" % (cons.TUCAN_NAME,"Select a Folder"), parent)
		...
		if files:
			self.set_title("%s - %s" % (cons.TUCAN_NAME, "Select Files"))
			self.set_action(gtk.FILE_CHOOSER_ACTION_OPEN)
			self.set_select_multiple(True)
		elif save:
			self.set_title("%s - %s" % (cons.TUCAN_NAME,"Save As"))
...
if __name__ == "__main__":
	def mierda(name):
		print name
	f = FileChooser(None, mierda, "/home/elie/")
